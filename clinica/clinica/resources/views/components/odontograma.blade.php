...
>
                            ...
                            <div class="flex items-stretch gap-0">
                                ...
                                <div class="flex-1 px-5 py-4">
                                    ...
                                    <p class="mt-1 text-sm text-[var(--brand-muted)]">
                                        <span class="font-semibold" x-text="pieza.tipo_legible"></span>
                                        <span> · </span>
                                        <span x-text="pieza.numero"></span>
                                        <span> · </span>
                                        <span x-text="pieza.numero_referencia"></span>
                                    </p>
    ...
    @else
        <div x-show="piezas.length > 0" x-cloak class="space-y-6">
            <div class="flex flex-col gap-3 rounded-xl border border-[var(--brand-border)] bg-[var(--brand-soft)]/30 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <p class="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--brand-muted)]">Cuadro activo</p>
                    <p class="mt-1 text-sm text-[var(--brand-primary)]">
                        <span x-text="tipoOdontograma === 'temporal' ? 'Odontograma de niño' : 'Odontograma de adulto'"></span>
                        <span class="text-[var(--brand-muted)]" x-text="tipoOdontograma === 'temporal' ? ' · Dentición temporal (20 piezas)' : ' · Numeración universal 1–32'"></span>
                    </p>
                </div>

                <div class="inline-flex w-full rounded-lg border border-[var(--brand-border)] bg-white p-1 sm:w-auto">
                    <button
                        type="button"
                        @click="cambiarTipo('permanente')"
                        :class="tipoOdontograma === 'permanente' ? 'bg-[var(--brand-primary)] text-[var(--brand-contrast)] shadow-sm' : 'text-[var(--brand-primary)] hover:bg-[var(--brand-soft)]'"
                        class="flex-1 rounded-md px-3 py-2 text-sm font-semibold transition sm:flex-none"
                    >
                        Adulto
                    </button>
                    <button
                        type="button"
                        @click="cambiarTipo('temporal')"
                        :class="tipoOdontograma === 'temporal' ? 'bg-[var(--brand-primary)] text-[var(--brand-contrast)] shadow-sm' : 'text-[var(--brand-primary)] hover:bg-[var(--brand-soft)]'"
                        class="flex-1 rounded-md px-3 py-2 text-sm font-semibold transition sm:flex-none"
                    >
                        Niño
                    </button>
            ...
            <p class="text-xs text-[var(--brand-muted)]">
                Haga clic sobre una pieza para registrar su estado.
            </p>
                    ...
                    <template x-for="pieza in resumenPiezas" :key="pieza.id">
                        <li class="flex items-start gap-2 text-sm">
                            ...
                            <span>
                                ...
                                <span class="text-xs text-[var(--brand-muted)]" x-text="pieza.numero_referencia"></span>
                                <span class="text-[var(--brand-muted)]"> — </span>
                                <span x-text="estadosMeta[pieza.estado].label"></span>
                ...
                >
                    <header class="flex items-start justify-between gap-3 border-b border-[var(--brand-border)] px-6 py-5">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-[var(--brand-muted)]" x-text="selected ? `${selected.tipo_legible} · ${selected.numero_referencia}` : ''"></p>
...
<script>
    function odontograma(config) {
        return {
            ...
            tipoOdontograma: config.initialType,
            ...
            cuadrante(n) {
                return this.piezas
                    .filter((pieza) => pieza.cuadrante === n)
            ...
            tituloPieza(pieza) {
                return `Pieza ${pieza.numero} · ${pieza.numero_referencia} · ${this.estadosMeta[pieza.estado].label}`;
            },
            ...
            get resumenPiezas() {
                return this.piezas
                    .filter((pieza) => pieza.estado !== 'sana')
                    .sort((a, b) => {
                        if (a.tipo !== b.tipo) {
            ...
            async cargar() {
                ...
                try {
                    ...
                    url.searchParams.set('tipo', this.esReadonly ? 'all' : this.tipoOdontograma);
            ...
            cambiarTipo(tipo) {
                if (this.esReadonly || this.tipoOdontograma === tipo) return;
            ...
            async guardar() {
                ...
                try {
                    const url = new URL(`${this.baseUrl}/${this.selected.id}`, window.location.origin);
                    url.searchParams.set('tipo', this.tipoOdontograma);
            ...
            notificarCambio() {
                window.dispatchEvent(new CustomEvent('odontograma-changed', {
                    detail: { piezas: this.piezas.map((pieza) => ({ id: pieza.id, estado: pieza.estado })) },
                }));
            },

            notificarTipo() {
                window.dispatchEvent(new CustomEvent('odontograma-type-changed', {
                    detail: { type: this.tipoOdontograma },
                }));
            },
